...
mounted (){
  this.element
    =document.querySelector(&#39;.liaoning-01&#39;);
  this.elementParent
    =document.querySelector(&#39;.liaoning-01&#39;).parentNode;
        this.load();
  this.onScroll();
},
methods: {
  ...
  clientHeight(){
    ...
    onScroll(){
      ...
      this.elementParent
        ...
        if(_this.scrollTop()+_this.clientHeight()-50
          &gt;=(_this.documentHeight())){
          setTimeout(function () {
            if (_this.number&lt;=2) {
              _this.loadDown();
                  _this.number++;
      ...
      setTimeout(function () {
		...
		<div class="vs">
			<ul>
				...
				<li>4.向上滑动时触发<code>scroll</code>事件，根据点2中的判断，加载更多的数据。其中数据来自方法<code>loadDown</code>中的<code>ajax</code>请求回来的数据，本例中为模拟数据。</li>
			</ul>
		</div>
	</div>
</template>
